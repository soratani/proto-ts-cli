...
export function replaceSamePath(base: string, file: string) {
  ...
  base = resolve(base);
  file = resolve(file);
  const a = base.split(sep);
  const b = file.split(sep);
  const index = a.findIndex((v, i) => b[i] !== v);
  if (~index) {
    return join(...b.slice(index));
  }
  return file;
}
...
export function firstLower(name: string) {
    return name.replace(name[0], name[0].toLowerCase());
}

export function firstUpper(name: string) {
